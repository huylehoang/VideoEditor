...
float4 motionBlurFrom(float2 _st,
                      float2 speed,
                      float ratio,
                      texture2d<float, access::sample> fromTexture)
{
  ...
  for (float t = 0.0; t <= 20.0; t++) {
    ...
    newuv = fract(newuv);
    color += getColor(newuv, fromTexture, ratio).rgb * weight;
    total += weight;
  }
...
float4 motionBlurTo(float2 _st,
                    float2 speed,
                    float ratio,
                    texture2d<float, access::sample> toTexture)
{
  ...
  for (float t = 0.0; t <= 20.0; t++) {
    ...
    float2 newuv = texCoord + speed * percent;
    newuv = fract(newuv);
    color += getColor(newuv, toTexture, ratio).rgb * weight;
    total += weight;
  }
  return float4(color / total, 1.0);
}
...
kernel void TangentMotionBlurTransition(texture2d<float, access::write> outputTexture [[ texture(0) ]],
                                        ...
                                        uint2 tpg [[ threads_per_grid ]])
{
  ...
  float4 outColor;
  if (easingTime <= .5) {
    outColor = motionBlurFrom(mystCurrent, speed, ratio, fromTexture);
  } else {
    outColor = motionBlurTo(mystCurrent, speed, ratio, toTexture);
  }
