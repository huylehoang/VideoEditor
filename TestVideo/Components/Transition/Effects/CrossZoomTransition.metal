...
float3 crossFade(float2 uv,
                 float dissolve,
                 float ratio,
                 texture2d<float, access::sample> fromTexture,
                 texture2d<float, access::sample> toTexture) {
    return mix(getColor(uv, fromTexture, ratio).rgb,
               getColor(uv, toTexture, ratio).rgb,
               dissolve);
}

kernel void CrossZoomTransition(texture2d<float, access::write> outputTexture [[ texture(0) ]],
                                ...
                                uint2 tpg [[ threads_per_grid ]])
{
  ...
  for (float t = 0.0; t <= 40.0; t++) {
      ...
      color += crossFade(texCoord + toCenter * percent * st, dissolve, ratio, fromTexture, toTexture) * weight;
      total += weight;
  }

  float4 outColor = float4(color / total, 1.0);
